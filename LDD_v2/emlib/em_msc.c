...
#else /* GCC */
static MSC_Status_TypeDef MSC_WriteWordI(uint32_t *address, void const *data,
		uint32_t numBytes, MSC_WriteStrategy_Typedef writeStrategy) __attribute__ ((section(".ram")));
__STATIC_INLINE MSC_Status_TypeDef MSC_LoadWriteData(uint32_t* data,
		uint32_t numWords, MSC_WriteStrategy_Typedef writeStrategy) __attribute__ ((section(".ram")));
...
#endif
__STATIC_INLINE MSC_Status_TypeDef MSC_LoadWriteData(uint32_t* data,
		uint32_t numWords, MSC_WriteStrategy_Typedef writeStrategy) {
	uint32_t timeOut;
	uint32_t wordIndex;
		...
		/**** Write strategy: mscWriteFast ****/
			...
			while (wordIndex < numWords) {
				...
				while (!(MSC ->STATUS & MSC_STATUS_WDATAREADY)) {
					 ...
					 */
					if ((MSC ->STATUS
							& (MSC_STATUS_WORDTIMEOUT | MSC_STATUS_BUSY
									| MSC_STATUS_WDATAREADY))== MSC_STATUS_WORDTIMEOUT) {
...
#endif
static MSC_Status_TypeDef MSC_WriteWordI(uint32_t *address, void const *data,
		uint32_t numBytes, MSC_WriteStrategy_Typedef writeStrategy) {
	uint32_t wordCount;
	uint32_t numWords;
	uint32_t pageWords;
	uint32_t* pData;
...
#pragma diag_suppress=Ta023
#endif
MSC_Status_TypeDef MSC_WriteWordFast(uint32_t *address, void const *data,
		uint32_t numBytes) {
